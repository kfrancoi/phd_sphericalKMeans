    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_KevGridExp.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * KevGridExp.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * KevGridExp.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * KevGridExp (application created by MATLAB Compiler)
        * run_KevGridExp.sh (shell script that can be run to 
    ...
    All Platforms:
        * KevGridExp.so (shared library, file extension varies by 
          platform)
        * KevGridExp.h (library header file)
        * KevGridExp.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * KevGridExp.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * KevGridExp.xml (documentation files)
        * KevGridExp.pdb (program database file - if DEBUG option 
          selected)
        * KevGridExp.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * KevGridExp.ctf (Component Technology File archive, contains 
          compiled M-code)
        * KevGridExp_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * KevGridExp.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * KevGridExp.ctf (Component Technology File archive, contains 
          compiled M-code)
        * KevGridExp.xla (Optionally created and placed in
